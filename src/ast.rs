...
pub enum Expression {
    ...
    IfExpr(Box<Node<IfExpr>>),
}

#[derive(Debug, PartialEq, Clone)]
pub struct SumTypeBuilder {
    pub sum_type: Node<Identifier>,
    pub variant: Node<Identifier>,
    pub inner_val: Box<Node<Expression>>,
}
...
// impl Deserialize for StorageClassSpecifier {
//     fn deserialize(&self) -> String {
//         match self {
...
#[derive(Debug, PartialEq, Clone)]
pub struct EnumInnerVal {
    pub type_specifier: Node<TypeSpecifier>,
}
...
pub struct Enumerator {
    pub identifier: Node<Identifier>,
    pub inner_val: Option<InnerType>,
...
pub struct ForEachStatement {
    pub elt: Node<Identifier>,
    pub r: Node<RangeType>,
...
// #[derive(Debug, PartialEq, Clone)]
// pub struct MatchStatement {
//     pub comparison: Node<Identifier>,
//     pub cases:
// }
...
pub struct IfLetStatement {
    pub sum_type: Node<Identifier>,
    pub variant: Node<Identifier>,
    pub inner_val_binding: Node<Identifier>,
    pub comparison: Node<Identifier>,
    pub then_statement: Box<Node<Statement>>,
    pub else_statement: Option<Box<Node<Statement>>>,
}
...
pub struct RangeType {
    pub start: Box<Node<Expression>>,
    pub stop: Box<Node<Expression>>,
    pub incr: Option<Box<Node<Expression>>>,
}

#[derive(Debug, PartialEq, Clone)]
pub struct IfExpr {
    pub cond: Box<Node<Expression>>,
    pub if_then: Vec<Box<Node<Statement>>>,
    pub if_ret: Box<Node<Expression>>,
    pub else_stmts: Vec<Box<Node<Statement>>>,
    pub else_ret: Box<Node<Expression>>,
    pub else_if: Vec<Node<ElseIf>>,
}
...
// Implement Deserialize for Identifier
...
// Implement Deserialize for Constant
impl Deserialize for Constant {
    fn deserialize(&self) -> String {
        match self {
            Constant::Integer(integer) => integer.deserialize(),
            Constant::Float(float) => float.deserialize(),
            Constant::Character(character) => format!("'{}'", character),
        }
    }
}

// Implement Deserialize for Integer
impl Deserialize for Integer {
    fn deserialize(&self) -> String {
        format!("{}{}", self.number, self.suffix.deserialize())
    }
}

// Implement Deserialize for IntegerSuffix
impl Deserialize for IntegerSuffix {
    fn deserialize(&self) -> String {
        ...
        if self.imaginary {
            suffix.push('i'); // or 'j' based on GNU extension
        }
        suffix
    }
}

// Implement Deserialize for Float
...
// Implement Deserialize for FloatSuffix
impl Deserialize for FloatSuffix {
    fn deserialize(&self) -> String {
        let mut suffix = String::new();
        match self.format {
            FloatFormat::Float => {
...
// Implement Deserialize for StringLiteral
impl Deserialize for StringLiteral {
    fn deserialize(&self) -> String {
        self.join(" ")
    }
}

impl Deserialize for MemberOperator {
    fn deserialize(&self) -> String {
        match self {
            Self::Direct => format!("."),
...
// Implement Deserialize for LabeledStatement (Example)

// Implement Deserialize for BlockItem (Example)
impl Deserialize for BlockItem {
    fn deserialize(&self) -> String {
        // Assuming BlockItem is a placeholder, implement appropriately
        "BlockItem".to_string()
    }
}

// Implement Deserialize for IfLetStatement (Example)
impl Deserialize for IfLetStatement {
    fn deserialize(&self) -> String {
        format!(
            "if let () ",
        )

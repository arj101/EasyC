...
impl<'ast, 'a> Visit<'ast> for Printer<'a> {
    ...
    fn visit_sum_type_builder(&mut self, s: &'ast SumTypeBuilder, _: &'ast Span) {
        self.name("SumTypeBuilder");
        visit_identifier(&mut self.block(),&s.sum_type.node, &s.sum_type.span)
    }
}
